...
Figure~\ref{fig:figure7} shows that $a_{1N_2}$ dominating the other species. This can be returned to the large value of $Y_{N_2}$, which mainly influences the $C_v$ mixture average rather than enthalpy and entropy. As $N_2$ is an inert species in $h_2v$ scheme. Same results can be found by analysing the other coefficients at low temperature.
...
Extracting the sensitivities with respect to NASA Polynomial coefficients at high temperature provides a similar  behaviour as it is shown in figure ~\ref{fig:figure8}. On the other hand, a comparison between figure~\ref{fig:figure7} and~\ref{fig:figure8} exhibits an increase in the gradient of $Y_{H_2O}$ which can be related to the increase in the value of $Y_{H_2O}$ due to the combustion process. This demonstrates that  thermodynamic properties for diluted mixture is controlled by the diluted species  
